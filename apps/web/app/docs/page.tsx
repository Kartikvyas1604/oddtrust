  ...
  return (
    <section className="py-12">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        {endpoints.map((ep) => (
          <div key={ep.name} className="bg-bg-raised border border-line-hairline rounded-lg p-6">
            <div className="mb-2">
              <span className="text-xs text-text-primary">{ep.name}</span>
            </div>
            <p className="font-mono text-[11px] text-pitch-green mb-2 break-all">{ep.url}</p>
            <p className="text-[11px] text-text-tertiary">{ep.desc}</p>
          </div>
        ))}
      </div>

      <div className="bg-bg-raised border border-line-hairline rounded-lg p-6 max-w-2xl">
